    ...
    while (counter < 1000 & halt < 1 ){
      ...
      if (regular.traps == T){
        ...
        while ( marker1 > n.traps){
          ...
          #centre the points on the polygon...
          delta <- as.data.frame(rgeos::gCentroid (poly1))
          delta.pts <- as.data.frame(rgeos::gCentroid (all.pts))

          delta <- delta - delta.pts

          all.pts <- maptools::elide(all.pts, shift=c(0+delta[,1], 0+delta[,2]))
