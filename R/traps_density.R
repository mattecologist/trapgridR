...
traps_per_hectare <- function(points=points,
  ...
  while (nrow(points) < round(round(sum(area(poly))/10000, 1)*traps.per.hectare, 0)){

    block_size = sqrt(sum(area(poly)))
...
traps_per_distance <- function(points=points,
  ...
  while (trap.dist > dist.thresh){
    ...
    if (trap.dist > dist.thresh){
      ...
      D2 <- reclassify(D, c(-Inf, dist.thresh, NA, dist.thresh, Inf, 1))

      poly.temp <- rasterToPolygons(D2, dissolve=T)


      if (sum(raster::area(poly.temp))/dist.thresh^2 > 2.5){
        points.temp <- (makegrid(poly.temp, cellsize = 10))
      } else{
        points.temp <- as.data.frame(spsample(poly.temp, 1, type="regular"))
