...
```
```{r plot of simulation results, fig.width=7, fig.height=7}
library (ggplot2)
...
orchard_layout <- ggplot (data=as.data.frame(outbreaks.data$traps_sp), aes(coords.x1, coords.x2)) +
  geom_point(size=2, shape=7)+
  geom_point(data=as.data.frame(outbreaks), aes(coords.x1, coords.x2), colour=cols[3], alpha=0.5)+
